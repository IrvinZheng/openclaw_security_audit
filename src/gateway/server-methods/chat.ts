...
import { getContentAuditApprovalManager } from "../content-audit-approval.js";
...
function hashAuditConfig(cfg: ContentAuditConfig | undefined): string {
  if (!cfg) return "";
  // Include labelConfigs in hash to detect policy changes
...
function getContentAuditService(cfg: { security?: { contentAudit?: ContentAuditConfig } }): ContentAuditService | null {
  ...
  // Re-create service if config changed
...
export const chatHandlers: GatewayRequestHandlers = {
  ...
  "chat.send": async ({ params, respond, context, client }) => {
    ...
    // Content audit will be performed on AI response, not user message
    ...
    try {
      ...
      void dispatchInboundMessage({
        ...
        .then(async () => {
          ...
          // Content audit check on AI response (always run, regardless of agentRunStarted)
          if (combinedReply) {
            const auditService = getContentAuditService(cfg);
            if (auditService?.isEnabled()) {
              try {
                const auditResult = await auditService.auditText(combinedReply);
                context.logGateway.info(
                  `[ContentAudit] AI响应审计: label=${auditResult.label} action=${auditResult.action} confidence=${auditResult.confidence}`,
                );

                if (auditResult.action === "block") {
                  // Block the response entirely
                  context.logGateway.info(`[ContentAudit] AI响应被阻止: ${auditResult.label}`);
                  broadcastChatFinal({
                    context,
                    runId: clientRunId,
                    sessionKey: p.sessionKey,
                    message: {
                      role: "assistant",
                      content: [{ type: "text", text: `[内容已被安全审核拦截: ${auditResult.description}]` }],
                      timestamp: Date.now(),
                      stopReason: "blocked",
                    },
                  });
                ...
                if (auditResult.action === "confirm") {
                  // Request user confirmation before showing response
                  const approvalManager = getContentAuditApprovalManager();
                  const approvalTimeoutMs = cfg.security?.contentAudit?.timeoutMs ?? 30000;
                  const approvalRecord = approvalManager.create(
                    {
                      userMessage: parsedMessage.slice(0, 500), // User's original message
                      aiResponse: combinedReply.slice(0, 1000), // AI's response (truncated for display)
                  ...
                  context.logGateway.info(
                    `[ContentAudit] Broadcasting approval request: id=${approvalRecord.id}`,
                  );
                  context.broadcast("contentAuditApproval", {
                    type: "request",
                    record: approvalRecord,
                  });

                  context.logGateway.info(
                    `[ContentAudit] 等待用户确认AI响应: ${auditResult.description} (${auditResult.label}) id=${approvalRecord.id}`,
                  );

                  const decision = await approvalManager.waitForDecision(approvalRecord, approvalTimeoutMs);

                  if (decision === "block" || decision === null) {
                    const reason = decision === null ? "确认超时" : "用户拒绝";
                    context.logGateway.info(`[ContentAudit] ${reason}: id=${approvalRecord.id}`);
                    broadcastChatFinal({
                      context,
                      runId: clientRunId,
                      sessionKey: p.sessionKey,
                      message: {
                        role: "assistant",
                        content: [{ type: "text", text: `[AI响应已被${reason === "确认超时" ? "超时" : ""}拦截: ${auditResult.description}]` }],
                        timestamp: Date.now(),
                        stopReason: "blocked",
                      },
                    });
                  ...
                  context.logGateway.info(`[ContentAudit] 用户已确认放行AI响应: id=${approvalRecord.id}`);
                }
              } catch (err) {
                context.logGateway.warn(`[ContentAudit] AI响应审计失败: ${formatForLog(err)}`);
              }
            }
          }

          // Only handle transcript and final broadcast if agent run was not started
          // (when agentRunStarted is true, the agent handles its own final message)
          if (!agentRunStarted) {
