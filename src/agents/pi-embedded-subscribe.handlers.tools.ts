...
export async function handleToolExecutionStart(
  ...
  try {
    const securityGateway = createSecurityGateway(config);
    if (securityGateway.isEnabled()) {
      const argsRecord = args && typeof args === "object" ? (args as Record<string, unknown>) : {};
      const sessionKey = (ctx.params as { sessionKey?: string }).sessionKey;
      const securityResult = await securityGateway.checkToolCall(
        toolName,
        argsRecord,
        sessionKey,
      );
      ctx.log.debug(
        `[security] 工具调用安全检查: tool=${toolName} riskLevel=${securityResult.riskLevel}`,
      );
      if (securityResult.riskLevel === "high") {
        ctx.log.warn(
          `[security] 高风险工具调用被拒绝: tool=${toolName} reason=${securityResult.reason || "高风险操作"}`,
        );
      } else if (securityResult.riskLevel === "medium") {
        ctx.log.warn(
          `[security] 中风险工具调用检测到: tool=${toolName} reason=${securityResult.reason || "需要用户确认"}`,
        );
      } else if (securityResult.riskLevel === "low") {
        ctx.log.debug(`[security] 低风险工具调用已记录: tool=${toolName}`);
      }
    }
  } catch (err) {
    const errorMessage = err instanceof Error ? err.message : String(err);
    ctx.log.warn(`[security] 工具调用安全检查失败: ${errorMessage}`);
  }
