...
function resolveChannelOrder(snapshot: ChannelsStatusSnapshot | null): ChannelKey[] {
  if (snapshot?.channelMeta?.length) {
    const allChannels = snapshot.channelMeta.map((entry) => entry.id) as ChannelKey[];
    // 优先显示飞书和钉钉
    const priorityChannels: ChannelKey[] = ["feishu", "dingtalk"];
    const priority = allChannels.filter((id) => priorityChannels.includes(id));
    const others = allChannels.filter((id) => !priorityChannels.includes(id));
