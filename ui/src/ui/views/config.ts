...
// Section definitions - key to i18n key mapping
const SECTION_KEYS = [
  { key: "env", labelKey: "environment" },
  { key: "update", labelKey: "updates" },
  { key: "agents", labelKey: "agents" },
  { key: "auth", labelKey: "authentication" },
  { key: "channels", labelKey: "channels" },
  { key: "messages", labelKey: "messages" },
  { key: "commands", labelKey: "commands" },
  { key: "hooks", labelKey: "hooks" },
  { key: "skills", labelKey: "skills" },
  { key: "tools", labelKey: "tools" },
  { key: "gateway", labelKey: "gateway" },
  { key: "wizard", labelKey: "setupWizard" },
] as const;

// Get localized sections
function getSections(lang: Lang): Array<{ key: string; label: string }> {
  const t = getSection(lang, "config");
  return SECTION_KEYS.map(({ key, labelKey }) => ({
    key,
...
export function renderConfig(props: ConfigProps) {
  ...
  const t = getSection(lang, "config");
  const validity =
    props.valid == null ? t.unknown : props.valid ? t.valid : t.invalid;
  ...
  const localizedSections = getSections(lang);
  const availableSections = localizedSections.filter(s => s.key in schemaProps);

  // Add any sections in schema but not in our list
  const knownKeys = new Set(localizedSections.map(s => s.key));
    ...
    <div class="config-layout">
      ...
      <aside class="config-sidebar">
        <div class="config-sidebar__header">
          ...
          <span class="pill pill--sm ${validity === t.valid ? "pill--ok" : validity === t.invalid ? "pill--danger" : ""}">${validity}</span>
        </div>
        ...
        <nav class="config-nav">
          ...
          >
            ...
            <span class="config-nav__label">${t.allSettings}</span>
          </button>
      ...
      <main class="config-main">
        ...
        <div class="config-actions">
          <div class="config-actions__left">
            ${hasChanges ? html`
              <span class="config-changes-badge">${props.formMode === "raw" ? t.unsavedChanges : `${diff.length} ${t.unsavedChanges}`}</span>
            ` : html`
              <span class="config-status muted">${t.noChanges}</span>
            `}
          </div>
        ...
        ${hasChanges && props.formMode === "form" ? html`
          <details class="config-diff">
            <summary class="config-diff__summary">
              <span>${t.viewPendingChanges} (${diff.length})</span>
