...
export function renderTab(state: AppViewState, tab: Tab) {
  ...
  return html`
    <a
      ...
      title=${titleForTab(tab, lang)}
    >
      <span class="nav-item__icon" aria-hidden="true">${icons[iconForTab(tab)]}</span>
      <span class="nav-item__text">${titleForTab(tab, lang)}</span>
    </a>
  `;
}
...
export function renderChatControls(state: AppViewState) {
  ...
  // Get active agent model name
  const activeModel = state.agentModelsData?.models?.find(
    (m) => m.id === state.agentModelsData?.activeModelId,
  );
  const lang = state.settings.lang;
  return html`
    <div class="chat-controls">
      ...
      <button
        class="btn btn--sm"
        @click=${() => state.setTab("models")}
        title=${lang === "zh" ? "Agent 模型管理" : "Agent Model Management"}
      >
        ${icons.box}
        ${activeModel ? html`<span class="chat-controls__agent-name">${activeModel.name}</span>` : ""}
      </button>
    </div>
  `;
}
